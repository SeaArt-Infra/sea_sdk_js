...
当前目录是本地包，可直接通过文件路径安装：

```bash
npm install ../seaart_sdk_js
```

发布到 npm 后可按包名安装：

```bash
npm install @seaverseai/seaart-sdk-js
```

## 初始化

```js
import { Client } from '@seaverseai/seaart-sdk-js';
...
```js
import { withHeader, withHeaders } from '@seaverseai/seaart-sdk-js';
...
```js
import { imageURL, newTask, text } from '@seaverseai/seaart-sdk-js';
...
```js
import { decode } from '@seaverseai/seaart-sdk-js';
...
```js
import { ResponsesStreamTextAssembler } from '@seaverseai/seaart-sdk-js';
...
```js
import { ErrAuth, SeaArtError } from '@seaverseai/seaart-sdk-js';
